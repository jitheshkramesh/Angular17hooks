...
import { Film, Vehicle, VehicleResponse } from "../interfaces/vehicle.interface";
import { Observable, catchError, filter, forkJoin, map, shareReplay, switchMap, throwError } from "rxjs";
import { toSignal, toObservable } from '@angular/core/rxjs-interop';
import { BYPASS_LOG } from "../shared/interceptor/headers.interceptor";
...
@Injectable({
  ...
  private vehicles$ = this.http.get<VehicleResponse>(this.url, { context: new HttpContext().set(BYPASS_LOG, true) }).pipe(
    map((data) =>
      data.results.map((v) => ({
        ...v,
  ...
  private vehicleFilms$ = toObservable(this.selectedVehicle).pipe(
    ...
    switchMap(vehicle =>
      forkJoin(vehicle.films.map(link =>
        this.http.get<Film>(link, { context: new HttpContext().set(BYPASS_LOG, true) })))
    )
  );
